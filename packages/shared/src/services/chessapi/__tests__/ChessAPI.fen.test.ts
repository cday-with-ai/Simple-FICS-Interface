...
describe('ChessEngine - FEN Handling', () => {
    ...
    describe('Position History', () => {
        ...
        it('should detect threefold repetition', () => {
            // Use a simple knight moving back and forth
            board.loadFen('4k3/8/8/8/8/8/8/4K1N1 w - - 0 1');

            // Move knight back and forth to create repetition
            board.makeMove('Nf3'); // Knight to f3
            board.makeMove('Kd7'); // Black king moves
            board.makeMove('Ng1'); // Knight back to g1 (position 1)
            board.makeMove('Ke7'); // Black king moves back

            board.makeMove('Nf3'); // Knight to f3
            board.makeMove('Kd7'); // Black king moves
            board.makeMove('Ng1'); // Knight back to g1 (position 2 - same as position 1)
            board.makeMove('Ke7'); // Black king moves back

            board.makeMove('Nf3'); // Knight to f3
            board.makeMove('Kd7'); // Black king moves
            board.makeMove('Ng1'); // Knight back to g1 (position 3 - third repetition)

            // Third repetition should be detected
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle positions with no legal moves', () => {
            // Black king trapped in corner by white queen and king
            board.loadFen('k7/8/KQ6/8/8/8/8/8 b - - 0 1');

            const moves = board.getLegalMoves();
            expect(moves.length).toBe(0);
            expect(board.isGameOver()).toBe(true);
            expect(board.getGameResult()).toBe(GameResult.DRAW); // Stalemate
        });
