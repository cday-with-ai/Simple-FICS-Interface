...
`,rr=m.button`
  ...
  `}
`,Mo=X(({isOpen:e,onClose:t})=>{const r=De(),{settingsRegistry:n}=r,o=Ne(),s=o.isMobileDevice||o.dimensions.width<768,[a,u]=p.useState("board"),[c,l]=p.useState(""),[d,h]=p.useState({}),[g,f]=p.useState({x:0,y:0}),[b,y]=p.useState(!1),[v,x]=p.useState({x:0,y:0}),C=p.useRef(null);if(p.useEffect(()=>{if(e&&C.current&&!s){const k=C.current.getBoundingClientRect();f({x:(window.innerWidth-k.width)/2,y:(window.innerHeight-k.height)/2})}},[e,s]),p.useEffect(()=>{if(!b)return;const k=O=>{f({x:O.clientX-v.x,y:O.clientY-v.y})},N=()=>{y(!1)};return document.addEventListener("mousemove",k),document.addEventListener("mouseup",N),()=>{document.removeEventListener("mousemove",k),document.removeEventListener("mouseup",N)}},[b,v]),!e)return null;const $=n.getAllCategories(),w=c?n.search(c):n.getByCategory(a),S=(k,N)=>{const O=n.get(k);if(O){if(O.validate){const I=O.validate(N);if(typeof I=="string"){h(_=>({..._,[k]:I}));return}else if(I===!1){h(_=>({..._,[k]:"Invalid value"}));return}}h(I=>{const _={...I};return delete _[k],_}),O.value=N,O.onChange?.(N),k in r.preferences&&r.updatePreference(k,N)}},P=k=>{const N=n.get(k);N&&S(k,N.defaultValue)},R=(k,N,O)=>{const I=JSON.parse(localStorage.getItem("customSounds")||"{}"),_=`custom_${k}_${Date.now()}`;I[_]={dataUrl:N,fileName:O,settingId:k,uploadDate:new Date().toISOString()},localStorage.setItem("customSounds",JSON.stringify(I)),S(k,_);const ee=n.get(k);if(ee&&ee.options){const ce={label:`Custom: ${O}`,value:_},Se=ee.options.filter(q=>!q.value.startsWith("custom_"));ee.options=[...Se,ce]}},F=k=>{if(!(!k||k==="none"))try{let N;if(k.startsWith("custom_")){const _=JSON.parse(localStorage.getItem("customSounds")||"{}")[k];if(_&&_.dataUrl)N=_.dataUrl;else{console.error("Custom sound not found:",k);return}}else N=`/sounds/${k}`;const O=new Audio(N);O.volume=.5,O.play().catch(I=>{console.error("Failed to play sound:",I)})}catch(N){console.error("Error playing sound:",N)}},B=k=>{s||(y(!0),x({x:k.clientX-g.x,y:k.clientY-g.y}))},V=k=>{switch(k.type){case"boolean":return i.jsx(bc,{type:"checkbox",checked:k.value,onChange:I=>S(k.id,I.target.checked),$isMobile:s});case"select":if(k.id.endsWith("SoundFile")){const I=k.options?.find(ce=>ce.value===k.value),_=I?I.label:"None",ee=k.value&&k.value!=="none";return i.jsxs(kc,{children:[i.jsx(jc,{children:_}),i.jsx(Pc,{type:"button",onClick:()=>F(k.value),disabled:!ee,title:ee?"Play sound":"No sound selected",children:"▶️ Play"}),i.jsx(nc,{settingId:k.id,onUpload:R})]})}else return i.jsx($c,{value:k.value,onChange:I=>S(k.id,I.target.value),children:k.options?.map(I=>i.jsx("option",{value:I.value,children:I.label},I.value))});case"number":return i.jsx(vc,{type:"number",value:k.value,min:k.min,max:k.max,step:k.step,onChange:I=>S(k.id,Number(I.target.value))});case"color":return i.jsx(wc,{type:"color",value:k.value,onChange:I=>S(k.id,I.target.value),$isMobile:s});case"text":const O=!!d[k.id];return i.jsxs("div",{style:{width:"100%"},children:[i.jsx(Sc,{value:k.value||"",onChange:I=>S(k.id,I.target.value),className:O?"error":"",placeholder:k.placeholder||"",spellCheck:!1}),O&&i.jsx(Cc,{children:d[k.id]})]});default:return null}};return i.jsx(oc,{children:i.jsxs(ic,{ref:C,$x:g.x,$y:g.y,$isMobile:s,children:[i.jsxs(sc,{onMouseDown:B,children:[i.jsx(ac,{children:"⚙️ Settings"}),i.jsx(cc,{onClick:t,onMouseDown:k=>k.stopPropagation(),children:"✕"})]}),i.jsx(lc,{children:i.jsx(dc,{type:"text",placeholder:"Search settings...",value:c,onChange:k=>l(k.target.value)})}),i.jsxs(uc,{$isMobile:s,children:[i.jsx(hc,{$isMobile:s,children:$.map(k=>i.jsxs(pc,{$active:a===k.id&&!c,$isMobile:s,onClick:()=>{u(k.id),l("")},children:[i.jsx(mc,{children:k.icon}),!s&&k.label]},k.id))}),i.jsxs(fc,{children:[c&&i.jsxs(Mc,{children:["Found ",w.length,' settings matching "',c,'"']}),i.jsx(gc,{children:w.map(k=>k.type==="text"?i.jsxs(on,{style:{flexDirection:"column",alignItems:"stretch"},children:[i.jsxs("div",{style:{marginBottom:"8px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"},children:[i.jsxs("div",{children:[i.jsx(sn,{children:k.label}),k.description&&i.jsx(an,{children:k.description})]}),k.value!==k.defaultValue&&i.jsx(cn,{type:"button",onClick:()=>P(k.id),title:"Reset to default",style:{marginLeft:"8px",marginTop:0},children:"↻"})]}),V(k)]},k.id):i.jsxs(on,{children:[i.jsxs(xc,{children:[i.jsx(sn,{children:k.label}),k.description&&i.jsx(an,{children:k.description})]}),i.jsxs(yc,{children:[V(k),k.value!==k.defaultValue&&i.jsx(cn,{type:"button",onClick:()=>P(k.id),title:"Reset to default",children:"↻"})]})]},k.id))})]})]}),i.jsxs(Rc,{children:[i.jsx(rr,{onClick:()=>r.resetToDefaults(),children:"Reset to Defaults"}),i.jsxs(Ec,{children:[i.jsx(rr,{onClick:t,children:"Cancel"}),i.jsx(rr,{$variant:"primary",onClick:t,children:"Done"})]})]})]})})});Mo.displayName="SettingsDialog";const zc=m.header`
  height: 44px;
  background-color: ${e=>e.theme.colors.surface};
  border-bottom: 1px solid ${e=>e.theme.colors.border};
  display: flex;
