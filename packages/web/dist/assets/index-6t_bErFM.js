...
`,rr=m.button`
  ...
  `}
`,Mo=X(({isOpen:e,onClose:t})=>{const r=De(),{settingsRegistry:n}=r,o=Ne(),s=o.isMobileDevice||o.dimensions.width<768,[a,u]=p.useState("board"),[c,l]=p.useState(""),[d,h]=p.useState({}),[g,f]=p.useState({x:0,y:0}),[b,y]=p.useState(!1),[v,x]=p.useState({x:0,y:0}),C=p.useRef(null);if(p.useEffect(()=>{if(e&&C.current&&!s){const k=C.current.getBoundingClientRect();f({x:(window.innerWidth-k.width)/2,y:(window.innerHeight-k.height)/2})}},[e,s]),p.useEffect(()=>{if(!b)return;const k=F=>{f({x:F.clientX-v.x,y:F.clientY-v.y})},O=()=>{y(!1)};return document.addEventListener("mousemove",k),document.addEventListener("mouseup",O),()=>{document.removeEventListener("mousemove",k),document.removeEventListener("mouseup",O)}},[b,v]),!e)return null;const $=n.getAllCategories(),w=c?n.search(c):n.getByCategory(a),S=(k,O)=>{const F=n.get(k);if(F){if(F.validate){const I=F.validate(O);if(typeof I=="string"){h(_=>({..._,[k]:I}));return}else if(I===!1){h(_=>({..._,[k]:"Invalid value"}));return}}h(I=>{const _={...I};return delete _[k],_}),F.value=O,F.onChange?.(O),k in r.preferences&&r.updatePreference(k,O)}},R=k=>{const O=n.get(k);O&&S(k,O.defaultValue)},P=(k,O,F)=>{const I=JSON.parse(localStorage.getItem("customSounds")||"{}"),_=`custom_${k}_${Date.now()}`;I[_]={dataUrl:O,fileName:F,settingId:k,uploadDate:new Date().toISOString()},localStorage.setItem("customSounds",JSON.stringify(I)),S(k,_);const ee=n.get(k);if(ee&&ee.options){const ce={label:`Custom: ${F}`,value:_},Se=ee.options.filter(q=>!q.value.startsWith("custom_"));ee.options=[...Se,ce]}},N=k=>{if(!(!k||k==="none"))try{let O;if(k.startsWith("custom_")){const _=JSON.parse(localStorage.getItem("customSounds")||"{}")[k];if(_&&_.dataUrl)O=_.dataUrl;else{console.error("Custom sound not found:",k);return}}else O=`/sounds/${k}`;const F=new Audio(O);F.volume=.5,F.play().catch(I=>{console.error("Failed to play sound:",I)})}catch(O){console.error("Error playing sound:",O)}},B=k=>{s||(y(!0),x({x:k.clientX-g.x,y:k.clientY-g.y}))},V=k=>{switch(k.type){case"boolean":return i.jsx(bc,{type:"checkbox",checked:k.value,onChange:I=>S(k.id,I.target.checked),$isMobile:s});case"select":if(k.id.endsWith("SoundFile")){const I=k.options?.find(ce=>ce.value===k.value),_=I?I.label:"None",ee=k.value&&k.value!=="none";return i.jsxs(kc,{children:[i.jsx(jc,{children:_}),i.jsx(Pc,{type:"button",onClick:()=>N(k.value),disabled:!ee,title:ee?"Play sound":"No sound selected",children:"▶️ Play"}),i.jsx(nc,{settingId:k.id,onUpload:P})]})}else return i.jsx($c,{value:k.value,onChange:I=>S(k.id,I.target.value),children:k.options?.map(I=>i.jsx("option",{value:I.value,children:I.label},I.value))});case"number":return i.jsx(vc,{type:"number",value:k.value,min:k.min,max:k.max,step:k.step,onChange:I=>S(k.id,Number(I.target.value))});case"color":return i.jsx(wc,{type:"color",value:k.value,onChange:I=>S(k.id,I.target.value),$isMobile:s});case"text":const F=!!d[k.id];return i.jsxs("div",{style:{width:"100%"},children:[i.jsx(Sc,{value:k.value||"",onChange:I=>S(k.id,I.target.value),className:F?"error":"",placeholder:k.placeholder||"",spellCheck:!1}),F&&i.jsx(Cc,{children:d[k.id]})]});default:return null}};return i.jsx(oc,{children:i.jsxs(ic,{ref:C,$x:g.x,$y:g.y,$isMobile:s,children:[i.jsxs(sc,{onMouseDown:B,children:[i.jsx(ac,{children:"⚙️ Settings"}),i.jsx(cc,{onClick:t,onMouseDown:k=>k.stopPropagation(),children:"✕"})]}),i.jsx(lc,{children:i.jsx(dc,{type:"text",placeholder:"Search settings...",value:c,onChange:k=>l(k.target.value)})}),i.jsxs(uc,{$isMobile:s,children:[i.jsx(hc,{$isMobile:s,children:$.map(k=>i.jsxs(pc,{$active:a===k.id&&!c,$isMobile:s,onClick:()=>{u(k.id),l("")},children:[i.jsx(mc,{children:k.icon}),!s&&k.label]},k.id))}),i.jsxs(fc,{children:[c&&i.jsxs(Mc,{children:["Found ",w.length,' settings matching "',c,'"']}),i.jsx(gc,{children:w.map(k=>k.type==="text"?i.jsxs(on,{style:{flexDirection:"column",alignItems:"stretch"},children:[i.jsxs("div",{style:{marginBottom:"8px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"},children:[i.jsxs("div",{children:[i.jsx(sn,{children:k.label}),k.description&&i.jsx(an,{children:k.description})]}),k.value!==k.defaultValue&&i.jsx(cn,{type:"button",onClick:()=>R(k.id),title:"Reset to default",style:{marginLeft:"8px",marginTop:0},children:"↻"})]}),V(k)]},k.id):i.jsxs(on,{children:[i.jsxs(xc,{children:[i.jsx(sn,{children:k.label}),k.description&&i.jsx(an,{children:k.description})]}),i.jsxs(yc,{children:[V(k),k.value!==k.defaultValue&&i.jsx(cn,{type:"button",onClick:()=>R(k.id),title:"Reset to default",children:"↻"})]})]},k.id))})]})]}),i.jsxs(Rc,{children:[i.jsx(rr,{onClick:()=>r.resetToDefaults(),children:"Reset to Defaults"}),i.jsxs(Ec,{children:[i.jsx(rr,{onClick:t,children:"Cancel"}),i.jsx(rr,{$variant:"primary",onClick:t,children:"Done"})]})]})]})})});Mo.displayName="SettingsDialog";const zc=m.header`
  height: 44px;
  background-color: ${e=>e.theme.colors.surface};
  border-bottom: 1px solid ${e=>e.theme.colors.border};
  display: flex;
...
`,xl=m(yt)`
    ...
    width: fit-content;
`,at=X(({player:e,isActive:t,size:r="small",compact:n=!0,variant:o="portrait"})=>{const s=Ot(),a=o==="landscape"?xl:gl;return i.jsx(a,{time:e.time,isActive:t,isFinished:s.isGameOver,showTenths:e.time<10,lowTimeThreshold:30,size:r,compact:n,"data-settings":"clock",className:"chess-clock"})});at.displayName="ObservableClock";const yl=m.div`
  position: relative;
  display: inline-block;
`,bl=m.button`
...
`,Go=X(({content:e,ansiColors:t=!0,elements:r=[]})=>{const{ficsStore:n,preferencesStore:o}=Ee(),s=o.getChatAppearance(),a=e.startsWith(`
`)?e.substring(1):e,u=d=>{if(!t)return d;const h={30:"#000000",31:"#CC0000",32:"#4E9A06",33:"#C4A000",34:"#3465A4",35:"#75507B",36:"#06989A",37:"#D3D7CF",90:"#555753",91:"#EF2929",92:"#8AE234",93:"#FCE94F",94:"#729FCF",95:"#AD7FA8",96:"#34E2E2",97:"#EEEEEC"};return d.replace(/\x1b\[(\d+)m/g,(g,f)=>{const b=h[f];return b?`<span style="color: ${b}">`:f==="0"?"</span>":""})},c=d=>{const h=[];if(d.match(/^\w+(?:\([^)]*\))*\s*tells\s+you:|^\s*\w+(?:\([^)]*\))*\((\d+)\):/m)){const f=/(https?:\/\/[^\s]+)\s*$/gm;let b;for(;(b=f.exec(d))!==null;){const y=b[1],v=b.index,x=b.index+b[0].length,C=d.substring(x).match(/^\n\s+([^\s]+)/);if(C&&C[1].match(/[.\/\-?=&]/)){const $=y+C[1],w=x+C[0].length;h.push({start:v,end:w,url:$})}}}return h},l=d=>{const h=u(d),g=c(e),f=[{regex:/(https?:\/\/[^\s]+)/g,handler:($,w)=>{const S=$[0],R=w||S;return i.jsx(Oe,{href:R,target:"_blank",rel:"noopener noreferrer",onClick:P=>{P.preventDefault(),window.open(R,"_blank")},children:S})}},{regex:/^(\w+) tells you:/gm,handler:$=>{const w=$[1];return i.jsxs("span",{children:[i.jsx(Oe,{onClick:S=>{S.preventDefault(),n.sendCommand(`finger ${w}`)},children:w}),$[0].substring(w.length)]},`player-${w}`)}},{regex:/\bGame (\d+)\b/g,handler:$=>i.jsx(Oe,{onClick:w=>{w.preventDefault(),n.sendCommand(`observe ${$[1]}`)},children:$[0]})}];if(t&&h!==d)return i.jsx("span",{dangerouslySetInnerHTML:{__html:h}});let b=0;const y=[],v=[],x=[];return r.forEach($=>{const w=a!==e?$.start-1:$.start;if(w>=0&&w<d.length){const S=$.type==="command"&&$.text.includes(":")&&($.text.match(/^\s*\d+\s+/)||$.text.match(/^%\d+:/)||$.text.match(/^\d+:/)),R=(()=>{const P=(()=>{switch($.type){case"command":return i.jsx(Oe,{onClick:N=>{N.preventDefault(),n.sendCommand($.action||$.value)},children:$.text});case"player":return i.jsx(Oe,{onClick:N=>{N.preventDefault(),n.sendCommand(`finger ${$.text}`)},children:$.text});case"gameNumber":return i.jsx(Oe,{onClick:N=>{N.preventDefault(),n.sendCommand(`observe ${$.value}`)},children:$.text});default:return $.text}})();return S?i.jsx(Cd,{children:P}):P})();x.push({start:w,end:w+$.text.length,render:R,priority:20})}}),g.forEach($=>{const w=d.substring($.start).match(/^(https?:\/\/[^\s]+)/);if(w){const S=w[1];x.push({start:$.start,end:$.start+S.length,render:i.jsx(Oe,{href:$.url,target:"_blank",rel:"noopener noreferrer",onClick:R=>{R.preventDefault(),window.open($.url,"_blank")},children:S}),priority:10}),v.push([$.start,$.end])}}),f.forEach($=>{const w=new RegExp($.regex);let S;for(;(S=w.exec(d))!==null;){const R=S.index,P=R+S[0].length;v.some(([B,V])=>R>=B&&R<V||P>B&&P<=V)||x.push({start:R,end:P,render:$.handler(S),priority:1})}}),x.sort(($,w)=>$.start!==w.start?$.start-w.start:w.priority-$.priority),x.filter(($,w)=>{if(w===0)return!0;const S=x[w-1];return $.start>=S.end}).forEach(($,w)=>{$.start>b&&y.push(d.substring(b,$.start)),y.push(i.jsx(Z.Fragment,{children:$.render},w)),b=$.end}),b<d.length&&y.push(d.substring(b)),y.length>0?y:d};return i.jsx(Sd,{$fontSize:s.fontSize,children:l(a)})}),jd=m.div`
  margin: 0;
`;class Pd extends Uo{constructor(){super(...arguments),this.type="default"}canRender(){return!0}render({message:t}){const r=t.metadata?.parsedMessage?.elements;return i.jsx(jd,{children:i.jsx(Go,{content:t.content,elements:r})})}}class Y extends Uo{render({message:t}){const r=t.metadata?.parsedMessage?.elements;return i.jsx(Go,{content:t.content,elements:r})}}class Rd extends Y{constructor(){super(...arguments),this.type="shout"}}class Ed extends Y{constructor(){super(...arguments),this.type="cshout"}}class Md extends Y{constructor(){super(...arguments),this.type="notification"}}class zd extends Y{constructor(){super(...arguments),this.type="seekAnnouncement"}}class Td extends Y{constructor(){super(...arguments),this.type="matchRequest"}}class Ld extends Y{constructor(){super(...arguments),this.type="illegalMove"}}class Id extends Y{constructor(){super(...arguments),this.type="drawOffer"}}class Ad extends Y{constructor(){super(...arguments),this.type="unobserve"}}class Dd extends Y{constructor(){super(...arguments),this.type="gameNotification"}}class Nd extends Y{constructor(){super(...arguments),this.type="whoOutput"}}class Od extends Y{constructor(){super(...arguments),this.type="gamesOutput"}}class Fd extends Y{constructor(){super(...arguments),this.type="fingerOutput"}}class Bd extends Y{constructor(){super(...arguments),this.type="historyOutput"}}class Wd extends Y{constructor(){super(...arguments),this.type="journalOutput"}}class Hd extends Y{constructor(){super(...arguments),this.type="soughtOutput"}}class _d extends Y{constructor(){super(...arguments),this.type="channelListOutput"}}class Ud extends Y{constructor(){super(...arguments),this.type="newsOutput"}}class Gd extends Y{constructor(){super(...arguments),this.type="inOutput"}}class Yd extends Y{constructor(){super(...arguments),this.type="login"}}class Vd extends Y{constructor(){super(...arguments),this.type="password"}}class qd extends Y{constructor(){super(...arguments),this.type="guestLoginConfirmation"}}class Kd extends Y{constructor(){super(...arguments),this.type="sessionStart"}}class Xd extends Y{constructor(){super(...arguments),this.type="system"}}class Qd extends Y{constructor(){super(...arguments),this.type="raw"}}function Jd(){H.clear(),H.register(new Rd),H.register(new Ed),H.register(new Md),H.register(new zd),H.register(new Td),H.register(new Ld),H.register(new Id),H.register(new Ad),H.register(new Dd),H.register(new Nd),H.register(new Od),H.register(new Fd),H.register(new Bd),H.register(new Wd),H.register(new Hd),H.register(new _d),H.register(new Ud),H.register(new Gd),H.register(new Yd),H.register(new Vd),H.register(new qd),H.register(new Kd),H.register(new Xd),H.register(new Qd),H.register(new Pd)}Jd();const lt=X(({message:e,currentUsername:t,onCommandClick:r,onHover:n})=>{const{preferencesStore:o}=Ee(),s=e.metadata?.consoleType,a=e.metadata?.channelNumber,u=s?o.getConsoleColor(s,a):null,c=s?o.getConsoleFont(s,a):null,l=s?o.getConsoleFontStyle(s,a):null,d={...e,metadata:{...e.metadata,color:u,fontFamily:c,fontStyle:l}},h=H.getRenderer(d);return h?i.jsx("div",{onMouseEnter:()=>n?.(e.timestamp),onMouseLeave:()=>n?.(null),children:h.render({message:d,currentUsername:t,onCommandClick:r,onHover:n})}):(console.warn("No renderer found for message:",e),i.jsx("div",{children:e.content}))});lt.displayName="Message";const kt=m.div`
  flex: 1;
  background-color: ${e=>e.theme.colors.background};
  border-radius: ${e=>e.theme.borderRadius.container};
...
`,Ko=X(()=>{const{preferencesStore:e}=Ee(),{viewMode:t,autoViewMode:r}=e.preferences,n=Ne(),o=Ro(),s=Eo(),a=Ja(),[u,c]=p.useState(600),[l,d]=p.useState(!1),h=p.useRef(!1);p.useEffect(()=>{!h.current&&r&&(h.current=!0,e.updatePreference("viewMode",a.viewMode),e.updatePreference("chessOrientation",a.orientation),e.updatePreference("autoViewMode",!1))},[r,a,e]),p.useEffect(()=>{o.includes(t)||e.updatePreference("viewMode","chess-only")},[o,t,e]),p.useEffect(()=>{const v=e.preferences.chessOrientation;s.includes(v)||e.updatePreference("chessOrientation","portrait")},[s,e]);const g=v=>{v.preventDefault(),d(!0)};p.useEffect(()=>{if(!l)return;const v=C=>{const $=window.innerWidth-C.clientX;c(Math.max(300,Math.min(600,$))),window.dispatchEvent(new Event("resize"))},x=()=>{d(!1)};return document.addEventListener("mousemove",v),document.addEventListener("mouseup",x),()=>{document.removeEventListener("mousemove",v),document.removeEventListener("mouseup",x)}},[l]);const f=t==="chess-only"||t==="chess-and-chat",b=t==="chat-only"||t==="chess-and-chat",y=t==="chess-and-chat"&&!n.isMobile;return i.jsxs(du,{children:[i.jsx(zo,{}),i.jsxs(uu,{children:[i.jsx(hu,{$isVisible:f,children:i.jsx(Ho,{hasChat:b,chatWidth:b&&!n.isMobile?u:0})}),y&&i.jsx(mu,{$isVisible:!0,onMouseDown:g,style:{cursor:l?"col-resize":"ew-resize"}}),i.jsx(pu,{$isVisible:b,$fullWidth:t==="chat-only",style:{width:t==="chat-only"?void 0:b&&!n.isMobile?`${u}px`:void 0},children:i.jsx(qo,{})})]})]})});Ko.displayName="AppLayout";const fu=Wa`
    * {
        margin: 0;
        padding: 0;
        box-sizing: border-box;
...
`,gu=()=>i.jsx(Zo,{children:i.jsxs(_a,{children:[i.jsx(fu,{}),i.jsx(Cs,{children:i.jsx(ts,{children:i.jsx(_n,{path:"/",element:i.jsx(ec,{children:i.jsx(Ko,{})})})})})]})}),Xo=document.getElementById("root");if(!Xo)throw new Error("Root element not found");const xu=zn(Xo);xu.render(i.jsx(gu,{}));
