...
`,jl=l.div`
  flex: 1;
  overflow: hidden;
`,Pl=({isOpen:e,onClose:t})=>{m.useEffect(()=>{const n=i=>{i.key==="Escape"&&e&&t()};if(e)return document.addEventListener("keydown",n),()=>document.removeEventListener("keydown",n)},[e,t]);const r=n=>{n.target===n.currentTarget&&t()};return e?o.jsx(vl,{$isOpen:e,onClick:r,children:o.jsxs(wl,{onClick:n=>n.stopPropagation(),children:[o.jsxs(Sl,{children:[o.jsx(kl,{children:"FICS Backend Data"}),o.jsx(Cl,{onClick:t,children:"✕"})]}),o.jsx(jl,{children:o.jsx($l,{})})]})}):null},Rl=l.header`
  height: 44px;
  background-color: ${e=>e.theme.colors.surface};
  border-bottom: 1px solid ${e=>e.theme.colors.border};
  display: flex;
...
`,Vl=l.div.attrs(e=>({style:{transform:`translate(
  ...
  justify-content: center;
  will-change: transform;
`,Je=["a","b","c","d","e","f","g","h"],Ze=["8","7","6","5","4","3","2","1"];function ql(e,t){return(e+t)%2===0}function Kl(e,t,r){const n=r?Je[7-e]:Je[e],i=r?Ze[7-t]:Ze[t];return`${n}${i}`}function Xl(e){const t=new Map,[r]=e.split(" ");return r.split("/").forEach((i,s)=>{let a=0;for(const u of i)if(u>="1"&&u<="8")a+=parseInt(u);else{const c=`${Je[a]}${Ze[s]}`;t.set(c,u),a++}}),t}const kr=q(({position:e,size:t,flipped:r=!1,showCoordinates:n=!0,onMove:i,onDrop:s,highlightedSquares:a=new Set,lastMove:u,interactive:c=!0,onSizeCalculated:d,selectedCapturedPiece:p,onCapturedPieceSelect:f})=>{Oe();const g=Le(),h=Ht(),$=m.useRef(null),[x,v]=m.useState(t||200),[y,S]=m.useState(null),[C,P]=m.useState(new Set),[k,E]=m.useState(null),[b,R]=m.useState([]),L=m.useRef(),[F,w]=m.useState(null),[N,H]=m.useState(!1),A=m.useMemo(()=>Xl(e),[e]),U=m.useRef(new Map);m.useRef(0);const oe=m.useCallback((j,D)=>{const T=Je.indexOf(j[0]),I=Ze.indexOf(j[1]),M=D/8,z=r?(7-T)*M:T*M,G=r?(7-I)*M:I*M;return{x:z,y:G}},[r]),ie=m.useCallback((j,D,T)=>{const I=j.toLowerCase()==="p",M=T[1];return I&&(M==="8"||M==="1")},[]),de=m.useCallback(j=>{j.preventDefault(),h.isPlaying&&h.clearPremove()},[h]);m.useEffect(()=>{if(t){v(t);return}const j=()=>{if(!$.current)return;const z=$.current.parentElement;if(!z)return;const{width:G,height:te}=z.getBoundingClientRect();$.current.getBoundingClientRect();const ae=16,ee=G-ae,we=te-ae,B=Math.floor(Math.min(ee,we)),le=Math.max(100,Math.floor(B/8)*8);le!==x&&v(le)},D=setTimeout(j,50);j();let T;const I=()=>{clearTimeout(T),T=setTimeout(j,100)};window.addEventListener("resize",I);let M=null;return $.current&&$.current.parentElement&&(M=new ResizeObserver(()=>{I()}),M.observe($.current.parentElement)),()=>{window.removeEventListener("resize",I),clearTimeout(T),clearTimeout(D),M&&M.disconnect()}},[t,x]),m.useEffect(()=>{d&&x>0&&d(x)},[x,d]);const K=x/8,ve=m.useMemo(()=>{if(!g.preferences.animateMoves)return!1;if(h.isPlaying){const j=h.currentGame,D=h.playingColor;if(j&&D){const T=D==="white"?j.white.time:j.black.time,I=g.preferences.disableAnimationsThreshold;if(T<I)return!1}}return!0},[g.preferences.animateMoves,g.preferences.disableAnimationsThreshold,h.isPlaying,h.currentGame,h.playingColor]),pe=m.useRef("");m.useEffect(()=>{if(R([]),!ve||N||h.isProcessingServerUpdate){U.current=new Map(A);return}const j=U.current;if(u){const{from:D,to:T}=u,I=`${e}-${D}-${T}`;if(pe.current===I){U.current=new Map(A);return}const M=j.get(D),z=A.get(T);if(M&&z===M&&!A.has(D)){if(h.isPlaying&&h.currentGame){const G=h.gameRelation===1,te=h.playingColor,ae=te==="white"&&h.currentGame.turn==="b"||te==="black"&&h.currentGame.turn==="w";if(G||ae){U.current=new Map(A),pe.current=I;return}}pe.current=I,setTimeout(()=>{R([{piece:M,from:D,to:T,startTime:Date.now()}])},0)}}U.current=new Map(A)},[A,u,ve,N,h.isProcessingServerUpdate,e,h]),m.useEffect(()=>{if(N){const j=setTimeout(()=>{H(!1)},50);return()=>clearTimeout(j)}},[e,N]),m.useEffect(()=>{if(b.length===0)return;const j=()=>{const D=Date.now(),T=g.preferences.animationDuration;R(I=>{const M=I.filter(z=>D-z.startTime<T);return M.length>0&&(L.current=requestAnimationFrame(j)),M})};return L.current=requestAnimationFrame(j),()=>{L.current&&cancelAnimationFrame(L.current)}},[b.length,g.preferences.animationDuration]),m.useEffect(()=>{if(p)try{const j=h.currentPosition;h.chessBoard.getFen()!==j&&h.chessBoard.loadFen(j);const T=h.chessBoard.getLegalMoves().filter(M=>M.from==="@"&&M.san.toLowerCase().startsWith(p.toLowerCase())),I=new Set(T.map(M=>M.to));P(I),S(null)}catch(j){console.error("Error getting drop moves:",j),P(new Set)}},[p,h]);const Q=m.useCallback((j,D)=>{if(!c)return;const T=A.get(j);if(p){C.has(j)?(s?.(p,j),f?.(null),P(new Set)):(f?.(null),P(new Set));return}if(y)if(C.has(j)){const I=A.get(y);if(I&&ie(I,y,j)){const M=I===I.toUpperCase()?"white":"black";if(h.isPlaying){const z=g.preferences.autoPromotionPiece;h.isMyTurn?(H(!0),i?.(y,j,z)):h.setPremove(y,j,z)}else{const z=D?.currentTarget.getBoundingClientRect();w({from:y,to:j,color:M,position:z?{x:z.left+z.width/2,y:z.top+z.height/2}:{x:window.innerWidth/2,y:window.innerHeight/2}})}}else h.isPlaying&&!h.isMyTurn?h.setPremove(y,j):(H(!0),i?.(y,j));S(null),P(new Set)}else if(j===y)S(null),P(new Set);else if(T)if(S(j),g.preferences.showLegalMoves)try{const I=h.currentPosition;h.chessBoard.getFen()!==I&&h.chessBoard.loadFen(I);const M=h.chessBoard.getLegalMoves(j),z=new Set(M.map(G=>G.to));P(z)}catch(I){console.error("Error getting legal moves:",I),P(new Set)}else P(new Set);else S(null),P(new Set);else if(T){S(j);try{const I=h.currentPosition;h.chessBoard.getFen()!==I&&h.chessBoard.loadFen(I);const M=T===T.toUpperCase(),z=h.chessBoard.getActiveColor();if(M&&z==="w"||!M&&z==="b")if(g.preferences.showLegalMoves){const te=h.chessBoard.getLegalMoves(j),ae=new Set(te.map(ee=>ee.to));P(ae)}else P(new Set);else P(new Set),S(null)}catch(I){console.error("Error getting legal moves:",I),P(new Set)}}},[y,C,A,i,s,c,ie,h,g.preferences.autoPromotionPiece,p,f]),fe=m.useCallback((j,D,T)=>{if(!c)return;const I=j.clientX,M=j.clientY;let z=!1,G=!1;const ae=j.currentTarget.getBoundingClientRect().width,ee=B=>{const le=Math.abs(B.clientX-I),xe=Math.abs(B.clientY-M);(le>3||xe>3)&&T&&!G?(z=!0,G=!0,ge(D,T,B,ae)):G&&E(ce=>ce?{...ce,x:B.clientX,y:B.clientY}:null)},we=B=>{document.removeEventListener("mousemove",ee),document.removeEventListener("mouseup",we),G?ue(B,D,T):z?(E(null),S(null),P(new Set)):Q(D,j)};document.addEventListener("mousemove",ee),document.addEventListener("mouseup",we)},[c,Q]),ge=m.useCallback((j,D,T,I)=>{if(S(j),g.preferences.showLegalMoves)try{const z=h.currentPosition;h.chessBoard.getFen()!==z&&h.chessBoard.loadFen(z);const G=D===D.toUpperCase(),te=h.chessBoard.getActiveColor();if(G&&te==="w"||!G&&te==="b"){const ee=h.chessBoard.getLegalMoves(j),we=new Set(ee.map(B=>B.to));P(we)}else P(new Set)}catch(z){console.error("Error getting legal moves for drag:",z),P(new Set)}else P(new Set);const M={piece:D,from:j,x:T.clientX,y:T.clientY,size:I};E(M)},[g.preferences.showLegalMoves,h]),ue=m.useCallback((j,D,T)=>{try{const z=document.elementsFromPoint(j.clientX,j.clientY).find(G=>G.getAttribute("data-square"))?.getAttribute("data-square");if(z&&z!==D)if(ie(T,D,z)){const G=T===T.toUpperCase()?"white":"black";if(h.isPlaying){const te=g.preferences.autoPromotionPiece;h.isMyTurn?(H(!0),i?.(D,z,te)):h.setPremove(D,z,te)}else w({from:D,to:z,color:G,position:{x:j.clientX,y:j.clientY}})}else h.isPlaying&&!h.isMyTurn?h.setPremove(D,z):(H(!0),i?.(D,z))}catch(I){console.error("Error in handleDragEnd:",I)}E(null),S(null),P(new Set)},[i,ie,h,g.preferences.autoPromotionPiece]),se=m.useMemo(()=>{const j=[];for(let D=0;D<8;D++)for(let T=0;T<8;T++){const I=ql(T,D),M=Kl(T,D,r),z=A.get(M),G=a.has(M),te=u&&(u.from===M||u.to===M),ae=y===M,ee=C.has(M),we=k?.from===M;b.some(ce=>ce.to===M);const B=b.some(ce=>ce.from===M),le=n&&D===7,xe=n&&T===0;j.push(o.jsxs(Gl,{"data-square":M,$isLight:I,$isHighlighted:G,$isLastMoveSquare:!!te,$isSelected:ae,$isPossibleMove:ee,onMouseDown:ce=>fe(ce,M,z),children:[z&&!we&&!B&&o.jsx(Ne,{piece:z,size:K},`${z}-${K}`),le&&o.jsx(xn,{$type:"file",$isLight:I,$size:K,"data-settings":"coordinates",className:"coordinate-label",children:r?Je[7-T]:Je[T]}),xe&&o.jsx(xn,{$type:"rank",$isLight:I,$size:K,"data-settings":"coordinates",className:"coordinate-label",children:r?Ze[7-D]:Ze[D]})]},M))}return j},[r,n,A,a,u,y,C,k,K,Q,fe]);return o.jsxs(o.Fragment,{children:[o.jsxs(_l,{ref:$,$size:x,onContextMenu:de,"data-settings":"board",className:"chess-board",children:[o.jsx(Ul,{children:se}),b.map((j,D)=>{const T=oe(j.from,x),I=oe(j.to,x),M=Date.now()-j.startTime,z=g.preferences.animationDuration,G=Math.min(M/z,1),ae=(ee=>ee<.5?4*ee*ee*ee:1-Math.pow(-2*ee+2,3)/2)(G);return o.jsx(Vl,{$fromX:T.x,$fromY:T.y,$toX:I.x,$toY:I.y,$progress:ae,$size:K,children:o.jsx(Ne,{piece:j.piece,size:K},`${j.piece}-${K}`)},`${j.from}-${j.to}-${j.startTime}`)})]}),k&&o.jsx(o.Fragment,{children:o.jsx(Yl,{$x:k.x,$y:k.y,$size:k.size,children:o.jsx(Ne,{piece:k.piece,size:k.size,isDragging:!0},`${k.piece}-${k.size}-dragging`)})}),F&&o.jsx(Ho,{isOpen:!0,color:F.color,position:F.position,onSelect:j=>{H(!0),i?.(F.from,F.to,j),w(null)},onCancel:()=>w(null)})]})});kr.displayName="ChessBoardWithPieces";const Ql=l.div`
...
`,Sn=l.div`
  ...
  transition: all 0.3s ease;
`,Go=q(({evaluation:e,percent:t,orientation:r="vertical",className:n})=>{const s=Wt().isBottomPlayerWinning;let a,u,c;if(t===50)a=47,u=6,c=47;else if(t>50){const p=t-50;a=50-p,u=p,c=50}else{const p=50-t;a=50,u=p,c=50-p}const d=t===50?"#FFC107":s?"#2E7D32":"#C62828";if(r==="vertical"){const p=t<20;return o.jsxs($n,{$orientation:r,className:n,children:[o.jsx(vn,{$orientation:r,children:e}),o.jsx($d,{$needsDarkText:p,children:e}),o.jsxs(wn,{$orientation:r,children:[o.jsx(Ct,{style:{height:`${a}%`}}),o.jsx(Sn,{$color:d,style:{height:`${u}%`}}),o.jsx(Ct,{style:{height:`${c}%`}})]})]})}else return o.jsxs($n,{$orientation:r,className:n,children:[o.jsx(vn,{$orientation:r,children:e}),o.jsxs(wn,{$orientation:r,children:[o.jsx(Ct,{style:{width:`${c}%`}}),o.jsx(Sn,{$color:d,style:{width:`${u}%`}}),o.jsx(Ct,{style:{width:`${a}%`}})]})]})});Go.displayName="EvaluationBar";const vd=l.div`
  display: flex;
  align-items: center;
...
`,ri=q(({className:e,compact:t=!1})=>{const{chatStore:r,ficsStore:n,preferencesStore:i}=Pe(),[s,a]=m.useState(""),[u,c]=m.useState(!1),[d,p]=m.useState(null);Z.useEffect(()=>{!n.connected&&!n.connecting&&(console.log("Auto-connecting to FICS..."),n.connect())},[n]),Z.useEffect(()=>{n.error&&r.addMessage("console",{channel:"console",sender:"System",content:`Error: ${n.error}`,timestamp:new Date,type:"system"})},[n.error,r]);const f=h=>{if(console.log("handleSendMessage called with:",h,"Length:",h.length),!h.trim())return;const $=h.split(`
