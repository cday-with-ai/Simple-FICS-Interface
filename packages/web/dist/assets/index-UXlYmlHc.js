...
`,lr=m.button`
  ...
  `}
`,Fo=K(({isOpen:e,onClose:t})=>{const r=Oe(),{settingsRegistry:n}=r,o=Fe(),s=o.isMobileDevice||o.dimensions.width<768,[a,p]=f.useState("board"),[c,d]=f.useState(""),[l,h]=f.useState({}),[g,u]=f.useState({x:0,y:0}),[b,y]=f.useState(!1),[$,x]=f.useState({x:0,y:0}),j=f.useRef(null);if(f.useEffect(()=>{if(e&&j.current&&!s){const v=j.current.getBoundingClientRect();u({x:(window.innerWidth-v.width)/2,y:(window.innerHeight-v.height)/2})}},[e,s]),f.useEffect(()=>{if(!b)return;const v=F=>{u({x:F.clientX-$.x,y:F.clientY-$.y})},O=()=>{y(!1)};return document.addEventListener("mousemove",v),document.addEventListener("mouseup",O),()=>{document.removeEventListener("mousemove",v),document.removeEventListener("mouseup",O)}},[b,$]),!e)return null;const C=n.getAllCategories(),k=c?n.search(c):n.getByCategory(a),S=(v,O)=>{const F=n.get(v);if(F){if(F.validate){const I=F.validate(O);if(typeof I=="string"){h(H=>({...H,[v]:I}));return}else if(I===!1){h(H=>({...H,[v]:"Invalid value"}));return}}h(I=>{const H={...I};return delete H[v],H}),F.value=O,F.onChange?.(O),v in r.preferences&&r.updatePreference(v,O)}},L=v=>{const O=n.get(v);O&&S(v,O.defaultValue)},E=(v,O,F)=>{const I=JSON.parse(localStorage.getItem("customSounds")||"{}"),H=`custom_${v}_${Date.now()}`;I[H]={dataUrl:O,fileName:F,settingId:v,uploadDate:new Date().toISOString()},localStorage.setItem("customSounds",JSON.stringify(I)),S(v,H);const ee=n.get(v);if(ee&&ee.options){const ce={label:`Custom: ${F}`,value:H},je=ee.options.filter(Y=>!Y.value.startsWith("custom_"));ee.options=[...je,ce]}},W=v=>{if(!(!v||v==="none"))try{let O;if(v.startsWith("custom_")){const H=JSON.parse(localStorage.getItem("customSounds")||"{}")[v];if(H&&H.dataUrl)O=H.dataUrl;else{console.error("Custom sound not found:",v);return}}else O=`/sounds/${v}`;const F=new Audio(O);F.volume=.5,F.play().catch(I=>{console.error("Failed to play sound:",I)})}catch(O){console.error("Error playing sound:",O)}},G=v=>{s||(y(!0),x({x:v.clientX-g.x,y:v.clientY-g.y}))},J=v=>{switch(v.type){case"boolean":return i.jsx(Cc,{type:"checkbox",checked:v.value,onChange:I=>S(v.id,I.target.checked),$isMobile:s});case"select":if(v.id.endsWith("SoundFile")){const I=v.options?.find(ce=>ce.value===v.value),H=I?I.label:"None",ee=v.value&&v.value!=="none";return i.jsxs(Mc,{children:[i.jsx(Tc,{children:H}),i.jsx(Lc,{type:"button",onClick:()=>W(v.value),disabled:!ee,title:ee?"Play sound":"No sound selected",children:"▶️ Play"}),i.jsx(lc,{settingId:v.id,onUpload:E})]})}else return i.jsx(jc,{value:v.value,onChange:I=>S(v.id,I.target.value),children:v.options?.map(I=>i.jsx("option",{value:I.value,children:I.label},I.value))});case"number":return i.jsx(Pc,{type:"number",value:v.value,min:v.min,max:v.max,step:v.step,onChange:I=>S(v.id,Number(I.target.value))});case"color":return i.jsx(Rc,{type:"color",value:v.value,onChange:I=>S(v.id,I.target.value),$isMobile:s});case"text":const F=!!l[v.id];return i.jsxs("div",{style:{width:"100%"},children:[i.jsx(Ec,{value:v.value||"",onChange:I=>S(v.id,I.target.value),className:F?"error":"",placeholder:v.placeholder||"",spellCheck:!1}),F&&i.jsx(zc,{children:l[v.id]})]});default:return null}};return i.jsx(dc,{children:i.jsxs(uc,{ref:j,$x:g.x,$y:g.y,$isMobile:s,children:[i.jsxs(hc,{onMouseDown:G,children:[i.jsx(pc,{children:"⚙️ Settings"}),i.jsx(mc,{onClick:t,onMouseDown:v=>v.stopPropagation(),children:"✕"})]}),i.jsx(fc,{children:i.jsx(gc,{type:"text",placeholder:"Search settings...",value:c,onChange:v=>d(v.target.value)})}),i.jsxs(xc,{$isMobile:s,children:[i.jsx(yc,{$isMobile:s,children:C.map(v=>i.jsxs(bc,{$active:a===v.id&&!c,$isMobile:s,onClick:()=>{p(v.id),d("")},children:[i.jsx($c,{children:v.icon}),!s&&v.label]},v.id))}),i.jsxs(vc,{children:[c&&i.jsxs(Dc,{children:["Found ",k.length,' settings matching "',c,'"']}),i.jsx(wc,{children:k.map(v=>v.type==="text"?i.jsxs(mn,{style:{flexDirection:"column",alignItems:"stretch"},children:[i.jsxs("div",{style:{marginBottom:"8px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"},children:[i.jsxs("div",{children:[i.jsx(fn,{children:v.label}),v.description&&i.jsx(gn,{children:v.description})]}),v.value!==v.defaultValue&&i.jsx(xn,{type:"button",onClick:()=>L(v.id),title:"Reset to default",style:{marginLeft:"8px",marginTop:0},children:"↻"})]}),J(v)]},v.id):i.jsxs(mn,{children:[i.jsxs(kc,{children:[i.jsx(fn,{children:v.label}),v.description&&i.jsx(gn,{children:v.description})]}),i.jsxs(Sc,{children:[J(v),v.value!==v.defaultValue&&i.jsx(xn,{type:"button",onClick:()=>L(v.id),title:"Reset to default",children:"↻"})]})]},v.id))})]})]}),i.jsxs(Ic,{children:[i.jsx(lr,{onClick:()=>r.resetToDefaults(),children:"Reset to Defaults"}),i.jsxs(Ac,{children:[i.jsx(lr,{onClick:t,children:"Cancel"}),i.jsx(lr,{$variant:"primary",onClick:t,children:"Done"})]})]})]})})});Fo.displayName="SettingsDialog";const Nc=m.header`
  height: 44px;
  background-color: ${e=>e.theme.colors.surface};
  border-bottom: 1px solid ${e=>e.theme.colors.border};
  display: flex;
...
`,Yc={K:"wK",Q:"wQ",R:"wR",B:"wB",N:"wN",P:"wP",k:"bK",q:"bQ",r:"bR",b:"bB",n:"bN",p:"bP"},Vc={K:"♔",Q:"♕",R:"♖",B:"♗",N:"♘",P:"♙",k:"♚",q:"♛",r:"♜",b:"♝",n:"♞",p:"♟"},qc=m.div`
  ...
  cursor: ${e=>e.$isDragging?"grabbing":"grab"};
  filter: ${e=>e.$isDragging?"drop-shadow(0 4px 8px rgba(0,0,0,0.3))":"none"};
`,Ne=K(({piece:e,size:t,isDragging:r=!1,style:n})=>{const o=Oe(),[s,a]=Q.useState(!1),p=Yc[e];if(!p)return null;const c=o.preferences.pieceSet,d=`/pieces/${c}/${p}.svg`;return Q.useEffect(()=>{a(!1)},[e,c]),s?i.jsx(qc,{className:"chess-piece-fallback",$isDragging:r,$size:t,style:n,"data-settings":"pieces",children:Vc[e]||e}):i.jsx(Uc,{className:"chess-piece",src:d,alt:p,$isDragging:r,draggable:!1,style:n,"data-settings":"pieces",onError:()=>a(!0)})});Ne.displayName="ChessPiece";const Kc=m.div`
...
`,rl=m.div.attrs(e=>({style:{transform:`translate(
  ...
  justify-content: center;
  will-change: transform;
`,Qe=["a","b","c","d","e","f","g","h"],Je=["8","7","6","5","4","3","2","1"];function nl(e,t){return(e+t)%2===0}function ol(e,t,r){const n=r?Qe[7-e]:Qe[e],o=r?Je[7-t]:Je[t];return`${n}${o}`}function il(e){const t=new Map,[r]=e.split(" ");return r.split("/").forEach((o,s)=>{let a=0;for(const p of o)if(p>="1"&&p<="8")a+=parseInt(p);else{const c=`${Qe[a]}${Je[s]}`;t.set(c,p),a++}}),t}const Pr=K(({position:e,size:t,flipped:r=!1,showCoordinates:n=!0,onMove:o,onDrop:s,highlightedSquares:a=new Set,lastMove:p,interactive:c=!0,onSizeCalculated:d,selectedCapturedPiece:l,onCapturedPieceSelect:h})=>{Fe();const g=Oe(),u=_t(),b=f.useRef(null),[y,$]=f.useState(t||200),[x,j]=f.useState(null),[C,k]=f.useState(new Set),[S,L]=f.useState(null),[E,W]=f.useState([]),G=f.useRef(),[J,v]=f.useState(null),[O,F]=f.useState(!1),I=f.useMemo(()=>il(e),[e]),H=f.useRef(new Map);f.useRef(0);const ee=f.useCallback((w,T)=>{const M=Qe.indexOf(w[0]),z=Je.indexOf(w[1]),P=T/8,R=r?(7-M)*P:M*P,_=r?(7-z)*P:z*P;return{x:R,y:_}},[r]),ce=f.useCallback((w,T,M)=>{const z=w.toLowerCase()==="p",P=M[1];return z&&(P==="8"||P==="1")},[]),je=f.useCallback(w=>{w.preventDefault(),u.isPlaying&&u.clearPremove()},[u]);f.useEffect(()=>{if(t){$(t);return}const w=()=>{if(!b.current)return;const R=b.current.parentElement;if(!R)return;const{width:_,height:re}=R.getBoundingClientRect();b.current.getBoundingClientRect();const le=16,A=_-le,ie=re-le,q=Math.floor(Math.min(A,ie)),ue=Math.max(100,Math.floor(q/8)*8);ue!==y&&$(ue)},T=setTimeout(w,50);w();let M;const z=()=>{clearTimeout(M),M=setTimeout(w,100)};window.addEventListener("resize",z);let P=null;return b.current&&b.current.parentElement&&(P=new ResizeObserver(()=>{z()}),P.observe(b.current.parentElement)),()=>{window.removeEventListener("resize",z),clearTimeout(M),clearTimeout(T),P&&P.disconnect()}},[t,y]),f.useEffect(()=>{d&&y>0&&d(y)},[y,d]);const Y=y/8,se=f.useMemo(()=>{if(!g.preferences.animateMoves)return!1;if(u.isPlaying){const w=u.currentGame,T=u.playingColor;if(w&&T){const M=T==="white"?w.white.time:w.black.time,z=g.preferences.disableAnimationsThreshold;if(M<z)return!1}}return!0},[g.preferences.animateMoves,g.preferences.disableAnimationsThreshold,u.isPlaying,u.currentGame,u.playingColor]),ae=f.useRef("");f.useEffect(()=>{if(W([]),!se||O||u.isProcessingServerUpdate){H.current=new Map(I);return}const w=H.current;if(p){const{from:T,to:M}=p,z=`${e}-${T}-${M}`;if(ae.current===z){H.current=new Map(I);return}const P=w.get(T),R=I.get(M);if(P&&R===P&&!I.has(T)){if(u.isPlaying&&u.currentGame){const _=u.gameRelation===1,re=u.playingColor,le=re==="white"&&u.currentGame.turn==="b"||re==="black"&&u.currentGame.turn==="w";if(_||le){H.current=new Map(I),ae.current=z;return}}ae.current=z,setTimeout(()=>{W([{piece:P,from:T,to:M,startTime:Date.now()}])},0)}}H.current=new Map(I)},[I,p,se,O,u.isProcessingServerUpdate,e,u]),f.useEffect(()=>{if(O){const w=setTimeout(()=>{F(!1)},50);return()=>clearTimeout(w)}},[e,O]),f.useEffect(()=>{if(E.length===0)return;const w=()=>{const T=Date.now(),M=g.preferences.animationDuration;W(z=>{const P=z.filter(R=>T-R.startTime<M);return P.length>0&&(G.current=requestAnimationFrame(w)),P})};return G.current=requestAnimationFrame(w),()=>{G.current&&cancelAnimationFrame(G.current)}},[E.length,g.preferences.animationDuration]),f.useEffect(()=>{if(l)try{const w=u.currentPosition;u.chessBoard.getFen()!==w&&u.chessBoard.loadFen(w);const M=u.chessBoard.getLegalMoves().filter(P=>P.from==="@"&&P.san.toLowerCase().startsWith(l.toLowerCase())),z=new Set(M.map(P=>P.to));k(z),j(null)}catch(w){console.error("Error getting drop moves:",w),k(new Set)}},[l,u]);const te=f.useCallback((w,T)=>{if(!c)return;const M=I.get(w);if(l){C.has(w)?(s?.(l,w),h?.(null),k(new Set)):(h?.(null),k(new Set));return}if(x)if(C.has(w)){const z=I.get(x);if(z&&ce(z,x,w)){const P=z===z.toUpperCase()?"white":"black";if(u.isPlaying){const R=g.preferences.autoPromotionPiece;u.isMyTurn?(F(!0),o?.(x,w,R)):u.setPremove(x,w,R)}else{const R=T?.currentTarget.getBoundingClientRect();v({from:x,to:w,color:P,position:R?{x:R.left+R.width/2,y:R.top+R.height/2}:{x:window.innerWidth/2,y:window.innerHeight/2}})}}else u.isPlaying&&!u.isMyTurn?u.setPremove(x,w):(F(!0),o?.(x,w));j(null),k(new Set)}else if(w===x)j(null),k(new Set);else if(M)if(j(w),g.preferences.showLegalMoves)try{const z=u.currentPosition;u.chessBoard.getFen()!==z&&u.chessBoard.loadFen(z);const P=u.chessBoard.getLegalMoves(w),R=new Set(P.map(_=>_.to));k(R)}catch(z){console.error("Error getting legal moves:",z),k(new Set)}else k(new Set);else j(null),k(new Set);else if(M){j(w);try{const z=u.currentPosition;u.chessBoard.getFen()!==z&&u.chessBoard.loadFen(z);const P=M===M.toUpperCase(),R=u.chessBoard.getActiveColor();if(P&&R==="w"||!P&&R==="b")if(g.preferences.showLegalMoves){const re=u.chessBoard.getLegalMoves(w),le=new Set(re.map(A=>A.to));k(le)}else k(new Set);else k(new Set),j(null)}catch(z){console.error("Error getting legal moves:",z),k(new Set)}}},[x,C,I,o,s,c,ce,u,g.preferences.autoPromotionPiece,l,h]),me=f.useCallback((w,T,M)=>{if(!c)return;const z=w.clientX,P=w.clientY;let R=!1,_=!1;const le=w.currentTarget.getBoundingClientRect().width,A=q=>{const ue=Math.abs(q.clientX-z),ct=Math.abs(q.clientY-P);(ue>3||ct>3)&&M&&!_?(R=!0,_=!0,de(T,M,q,le)):_&&L(Le=>Le?{...Le,x:q.clientX,y:q.clientY}:null)},ie=q=>{document.removeEventListener("mousemove",A),document.removeEventListener("mouseup",ie),_?Pe(q,T,M):R?(L(null),j(null),k(new Set)):te(T,w)};document.addEventListener("mousemove",A),document.addEventListener("mouseup",ie)},[c,te]),de=f.useCallback((w,T,M,z)=>{if(j(w),g.preferences.showLegalMoves)try{const R=u.currentPosition;u.chessBoard.getFen()!==R&&u.chessBoard.loadFen(R);const _=T===T.toUpperCase(),re=u.chessBoard.getActiveColor();if(_&&re==="w"||!_&&re==="b"){const A=u.chessBoard.getLegalMoves(w),ie=new Set(A.map(q=>q.to));k(ie)}else k(new Set)}catch(R){console.error("Error getting legal moves for drag:",R),k(new Set)}else k(new Set);const P={piece:T,from:w,x:M.clientX,y:M.clientY,size:z};L(P)},[g.preferences.showLegalMoves,u]),Pe=f.useCallback((w,T,M)=>{try{const R=document.elementsFromPoint(w.clientX,w.clientY).find(_=>_.getAttribute("data-square"))?.getAttribute("data-square");if(R&&R!==T)if(ce(M,T,R)){const _=M===M.toUpperCase()?"white":"black";if(u.isPlaying){const re=g.preferences.autoPromotionPiece;u.isMyTurn?(F(!0),o?.(T,R,re)):u.setPremove(T,R,re)}else v({from:T,to:R,color:_,position:{x:w.clientX,y:w.clientY}})}else u.isPlaying&&!u.isMyTurn?u.setPremove(T,R):(F(!0),o?.(T,R))}catch(z){console.error("Error in handleDragEnd:",z)}L(null),j(null),k(new Set)},[o,ce,u,g.preferences.autoPromotionPiece]),fe=f.useMemo(()=>{const w=[];for(let T=0;T<8;T++)for(let M=0;M<8;M++){const z=nl(M,T),P=ol(M,T,r),R=I.get(P),_=a.has(P),re=p&&(p.from===P||p.to===P),le=x===P,A=C.has(P),ie=S?.from===P;E.some(Le=>Le.to===P);const q=E.some(Le=>Le.from===P),ue=n&&T===7,ct=n&&M===0;w.push(i.jsxs(el,{"data-square":P,$isLight:z,$isHighlighted:_,$isLastMoveSquare:!!re,$isSelected:le,$isPossibleMove:A,onMouseDown:Le=>me(Le,P,R),children:[R&&!ie&&!q&&i.jsx(Ne,{piece:R,size:Y},`${R}-${Y}`),ue&&i.jsx(bn,{$type:"file",$isLight:z,$size:Y,"data-settings":"coordinates",className:"coordinate-label",children:r?Qe[7-M]:Qe[M]}),ct&&i.jsx(bn,{$type:"rank",$isLight:z,$size:Y,"data-settings":"coordinates",className:"coordinate-label",children:r?Je[7-T]:Je[T]})]},P))}return w},[r,n,I,a,p,x,C,S,Y,te,me]);return i.jsxs(i.Fragment,{children:[i.jsxs(Jc,{ref:b,$size:y,onContextMenu:je,"data-settings":"board",className:"chess-board",children:[i.jsx(Zc,{children:fe}),E.map((w,T)=>{const M=ee(w.from,y),z=ee(w.to,y),P=Date.now()-w.startTime,R=g.preferences.animationDuration,_=Math.min(P/R,1),le=(A=>A<.5?4*A*A*A:1-Math.pow(-2*A+2,3)/2)(_);return i.jsx(rl,{$fromX:M.x,$fromY:M.y,$toX:z.x,$toY:z.y,$progress:le,$size:Y,children:i.jsx(Ne,{piece:w.piece,size:Y},`${w.piece}-${Y}`)},`${w.from}-${w.to}-${w.startTime}`)})]}),S&&i.jsx(i.Fragment,{children:i.jsx(tl,{$x:S.x,$y:S.y,$size:S.size,children:i.jsx(Ne,{piece:S.piece,size:S.size,isDragging:!0},`${S.piece}-${S.size}-dragging`)})}),J&&i.jsx(Wo,{isOpen:!0,color:J.color,position:J.position,onSelect:w=>{F(!0),o?.(J.from,J.to,w),v(null)},onCancel:()=>v(null)})]})});Pr.displayName="ChessBoardWithPieces";const sl=m.div`
...
`;const ut=K(({name:e,rating:t,time:r,isActive:n,isWhite:o,orientation:s="horizontal",hideClockInCard:a=!1,onlyInfo:p=!1,compact:c=!1})=>{const d=i.jsxs(i.Fragment,{children:[i.jsx(fl,{children:i.jsxs(gl,{children:[i.jsx(xl,{children:i.jsx(at,{name:e})}),i.jsx(yl,{children:t})]})}),!a&&!p&&i.jsx(kt,{time:r,isActive:n,showTenths:r<10,lowTimeThreshold:30,size:s==="horizontal"?"medium":"small"})]});return p?d:i.jsx(ml,{$isActive:n,$orientation:s,$compact:c,children:d})});ut.displayName="PlayerCard";const bl=m.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  background-color: ${e=>e.theme.colors.surface};
...
`,Rr=K(({orientation:e="vertical",boardSize:t})=>{const r=Gt();return i.jsx(El,{$orientation:e,$boardSize:t,children:i.jsx(Uo,{evaluation:r.evaluation,percent:r.evaluationPercent,orientation:e})})}),Mr=K(({className:e})=>{const t=Gt();return i.jsxs(zl,{className:e,children:[i.jsxs("div",{className:"depth",children:["Depth ",t.depth||0]}),i.jsx("div",{className:"line",children:t.principalVariation||(t.currentLine?t.currentLine.pv.join(" "):null)||"Calculating..."})]})});Rr.displayName="AnalysisDisplay";Mr.displayName="AnalysisInfoDisplay";const Tl=m.div`
  display: ${e=>e.$isOpen?"flex":"none"};
  position: fixed;
  top: 0;
  left: 0;
...
`,Xo=K(({className:e,hasChat:t=!1,chatWidth:r=0})=>{const n=_t(),o=Oe(),s=Gt(),a=Fn(),p=oi();Fe();const[c,d]=f.useState(!1),[l,h]=f.useState(!1),[g,u]=f.useState(0),[b,y]=f.useState(!1),[$,x]=f.useState(!1),[j,C]=f.useState(null),k=o.preferences.chessOrientation==="landscape",S=n.currentPosition||"rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",E=window.innerWidth/window.innerHeight>1.6,W=f.useMemo(()=>!n.currentGame||n.currentGame.gameId<0?"freestyle":n.isPlaying?"playing":n.isObserving?"observing":n.isExamining?"examining":"observing",[n.currentGame,n.gameRelation]),G=f.useMemo(()=>n.currentGame?.variant==="crazyhouse"?!0:o.preferences.showCapturedPieces,[n.currentGame?.variant,o.preferences.showCapturedPieces]),J=f.useCallback((A,ie,q)=>{try{n.makeMove(A,ie,q)||(console.error("Invalid move:",A,ie),p.playIllegal())}catch(ue){console.error("Error making move:",ue),p.playIllegal()}},[n,p]),v=f.useCallback((A,ie)=>{try{const q=A.toLowerCase();n.makeSANMove(`${A.toUpperCase()}@${ie}`)||(console.error("Invalid drop:",A,ie),p.playIllegal())}catch(q){console.error("Error making drop:",q),p.playIllegal()}},[n,p]),O=f.useCallback(A=>{C(j===A?null:A)},[j]);f.useMemo(()=>{if(n.currentGameInfo){const{white:A,black:ie,timeControl:q,variant:ue}=n.currentGameInfo;return`Game ${n.currentGame?.gameId||"?"} • ${ue} ${q}`}return"No active game"},[n.currentGameInfo,n.currentGame]);const F=(()=>{const A=n.moveHistory.length;if(A>0){const ie=n.moveHistory[A-1],q=Math.ceil(A/2),ue=A%2===1,ct=fr(ie.san);return`${q}.${ue?"":".."} ${ct}`}return"Starting position"})(),I=n.currentOpening,H=n.currentGame,ee=H||n.lastGameState,ce=ee?.white||{name:"White",rating:1500,time:900},je=ee?.black||{name:"Black",rating:1500,time:900},Y=!H||H.turn==="w",se=n.shouldShowFlippedBoard,ae=se?ce:je,te=se?je:ce,me=se,de=se?Y:!Y,Pe=f.useCallback(A=>{n.goToMove(A)},[n]);f.useEffect(()=>{s.initialize()},[s]),f.useEffect(()=>{$&&n.isPlaying&&n.currentGame&&a.sendCommand("draw")},[n.moveHistory.length,$,n.isPlaying,a]),f.useEffect(()=>{(!n.currentGame||!n.isPlaying)&&x(!1)},[n.currentGame,n.isPlaying]),f.useEffect(()=>{c&&s.isEngineReady?s.startAnalysis(S):s.stopAnalysis()},[c,S,s]);const fe=f.useCallback(()=>{d(A=>!A)},[]),w=f.useCallback(()=>{h(!0)},[]),T=f.useCallback(()=>{o.updatePreference("boardFlipped",!o.preferences.boardFlipped)},[o]),M=f.useCallback(()=>{n.currentGame&&a.sendCommand(`unobs ${n.currentGame.gameId}`)},[a,n.currentGame]),z=f.useCallback(()=>{a.sendCommand("unexamine")},[a]),P=f.useCallback(()=>{y(!0)},[]),R=f.useCallback(()=>{a.sendCommand("resign"),y(!1)},[a]),_=f.useCallback(()=>{a.sendCommand("draw"),x(!$)},[a,$]),re=f.useCallback(()=>{a.sendCommand("abort")},[a]),le=()=>i.jsxs(i.Fragment,{children:[i.jsx(En,{$orientation:"portrait",children:i.jsx(ad,{children:i.jsxs(cd,{children:[c&&i.jsx($d,{$boardSize:g,children:i.jsx(Rr,{orientation:"vertical",boardSize:g})}),i.jsx(ld,{children:i.jsxs("div",{style:{display:"flex",flexDirection:"column",alignItems:"center"},id:"board-container",children:[i.jsxs(od,{children:[i.jsxs(zn,{children:["Game #",ee?.gameId||"?"]}),i.jsx(Tn,{children:ee?.timeControl||"?"}),i.jsxs(id,{children:[W==="playing"&&i.jsxs(i.Fragment,{children:[n.moveHistory.length<=1&&i.jsx($e,{onClick:re,$variant:"secondary",children:"Abort"}),i.jsx($e,{onClick:_,$variant:"secondary",children:"Draw"}),i.jsx($e,{onClick:P,$variant:"secondary",children:"Resign"}),i.jsx(Hr,{color:n.playingColor||"white",size:"small"})]}),W==="observing"&&i.jsxs(i.Fragment,{children:[i.jsx($e,{onClick:M,$variant:"secondary",children:"Unobserve"}),i.jsx($e,{onClick:fe,$variant:"secondary",children:"Analysis"})]}),W==="examining"&&i.jsxs(i.Fragment,{children:[i.jsx($e,{onClick:z,$variant:"secondary",children:"Unexamine"}),i.jsx($e,{onClick:fe,$variant:"secondary",children:"Analysis"})]}),W==="freestyle"&&i.jsxs(i.Fragment,{children:[i.jsx($e,{onClick:fe,$variant:"secondary",children:"Analysis"}),i.jsx($e,{onClick:T,$variant:"secondary",children:"Flip"}),i.jsx($e,{onClick:w,$variant:"secondary",children:"FEN"})]})]})]}),i.jsxs(Dn,{children:[i.jsx(ht,{player:ae,isActive:de,size:"small",compact:!0}),i.jsx(Nn,{children:i.jsx(ut,{name:ae.name,rating:ae.rating,time:0,isActive:de,isWhite:me,orientation:"horizontal",hideClockInCard:!0,compact:!0})})]}),i.jsx(An,{$orientation:"portrait",children:i.jsx(Pr,{position:S,flipped:se,showCoordinates:o.preferences.showCoordinates,onMove:J,onDrop:v,interactive:W==="playing"||W==="freestyle"||W==="examining",lastMove:n.lastMove||void 0,onSizeCalculated:u,selectedCapturedPiece:j,onCapturedPieceSelect:C})}),i.jsxs(Dn,{children:[i.jsx(ht,{player:te,isActive:!de,size:"small",compact:!0}),i.jsx(Nn,{children:i.jsx(ut,{name:te.name,rating:te.rating,time:0,isActive:!de,isWhite:!me,orientation:"horizontal",hideClockInCard:!0,compact:!0})})]}),i.jsxs(sd,{children:[i.jsx(Ln,{children:n.premove?`Premove: ${Ur(`${n.premove.from}${n.premove.to}${n.premove.promotion||""}`,S)}`:F!=="Starting position"?`Last move: ${F}`:"Last move: none"}),I&&i.jsx(In,{children:I})]}),c&&i.jsx(bd,{children:i.jsx(Mr,{})})]})}),G&&i.jsx(ed,{$squareSize:g?g/8:0,children:i.jsxs(td,{$squareSize:g?g/8:0,children:[i.jsx(pt,{orientation:"vertical",isWhitePieces:se,boardSize:g,onPieceClick:O}),i.jsx(pt,{orientation:"vertical",isWhitePieces:!se,boardSize:g,onPieceClick:O})]})})]})})}),i.jsx(dd,{$orientation:"portrait",$boardSize:g,children:i.jsx(Zt,{moves:n.moveHistory,currentMoveIndex:n.currentMoveIndex,onMoveClick:Pe,disableAutoScroll:!0,onNavigate:A=>{if(n.isExamining)switch(A){case"first":a.sendCommand("back 500");break;case"prev":a.sendCommand("back");break;case"next":a.sendCommand("forward");break;case"last":a.sendCommand("forward 500");break}else switch(A){case"first":n.goToStart();break;case"prev":n.goToPreviousMove();break;case"next":n.goToNextMove();break;case"last":n.goToEnd();break}}})})]});return i.jsxs(Ql,{className:e,$orientation:k?"landscape":"portrait",$hasChat:t,children:[k?i.jsx(i.Fragment,{children:i.jsx(En,{$orientation:"landscape",children:i.jsxs(ud,{children:[i.jsx(xd,{children:c&&i.jsx(yd,{$boardSize:g,children:i.jsx(Rr,{orientation:"vertical",boardSize:g})})}),i.jsxs(hd,{$hasAnalysis:c,children:[i.jsxs(Jl,{$isWideAspect:E,children:[i.jsxs(rd,{$chatWidth:r,$hasAnalysis:c,children:[i.jsxs(zn,{children:["Game #",ee?.gameId||"?"]}),i.jsx(Tn,{children:ee?.timeControl||"?"})]}),i.jsx(Zl,{$orientation:"landscape",children:i.jsx(An,{$orientation:"landscape",$chatWidth:r,$hasAnalysis:c,children:i.jsx(Pr,{position:S,flipped:se,showCoordinates:o.preferences.showCoordinates,onMove:J,onDrop:v,interactive:W==="playing"||W==="freestyle"||W==="examining",lastMove:n.lastMove||void 0,onSizeCalculated:u,selectedCapturedPiece:j,onCapturedPieceSelect:C})})}),i.jsxs(nd,{$chatWidth:r,$hasAnalysis:c,children:[i.jsx(Ln,{children:n.premove?`Premove: ${Ur(`${n.premove.from}${n.premove.to}${n.premove.promotion||""}`,S)}`:F!=="Starting position"?`Last move: ${F}`:"Last move: none"}),I&&i.jsx(In,{children:I})]}),c&&i.jsx(gd,{$chatWidth:r,$hasAnalysis:c,children:i.jsx(Mr,{})})]}),i.jsxs(pd,{$isWideAspect:E,$boardSize:g,children:[G&&i.jsx(pt,{orientation:"horizontal",isWhitePieces:me,boardSize:g,onPieceClick:O}),i.jsx(ht,{player:ae,isActive:de,size:"small",compact:!0,variant:"landscape"}),i.jsxs(md,{children:[i.jsx(ut,{name:ae.name,rating:ae.rating,time:0,isActive:de,isWhite:me,orientation:"vertical",hideClockInCard:!0,compact:!0}),i.jsx(Go,{perspective:W,canAbort:n.moveHistory.length<=1,onAnalysis:fe,onFlipBoard:T,onSetupFEN:w,onUnobserve:M,onUnexamine:z,onResign:P,onDraw:_,onAbort:re,isAnalysisActive:c,isDrawOffered:$}),i.jsx(fd,{moves:n.moveHistory,currentMoveIndex:n.currentMoveIndex,onMoveClick:Pe,showHeader:!1,onNavigate:A=>{if(n.isExamining)switch(A){case"first":a.sendCommand("backward 999");break;case"prev":a.sendCommand("backward");break;case"next":a.sendCommand("forward");break;case"last":a.sendCommand("forward 999");break}else switch(A){case"first":n.goToStart();break;case"prev":n.goToPreviousMove();break;case"next":n.goToNextMove();break;case"last":n.goToEnd();break}}}),i.jsx(ut,{name:te.name,rating:te.rating,time:0,isActive:!de,isWhite:!me,orientation:"vertical",hideClockInCard:!0,compact:!0})]}),i.jsx(ht,{player:te,isActive:!de,size:"small",compact:!0,variant:"landscape"}),G&&i.jsx(pt,{orientation:"horizontal",isWhitePieces:!me,boardSize:g,onPieceClick:O})]})]})]})})}):le(),i.jsx(Vo,{isOpen:l,onClose:()=>h(!1)}),i.jsx(Xl,{isOpen:b,title:"Resign Game",message:"Are you sure you want to resign?",confirmText:"Yes, Resign",cancelText:"Cancel",onConfirm:R,onCancel:()=>y(!1)})]})});Xo.displayName="ChessGameLayout";const vd=m.div`
  display: flex;
  background-color: ${e=>e.theme.colors.surface};
  border-bottom: 1px solid ${e=>e.theme.colors.border};
  overflow-x: auto;
...
`,ei=K(({className:e,compact:t=!1})=>{const{chatStore:r,ficsStore:n,preferencesStore:o}=ze(),[s,a]=f.useState(""),[p,c]=f.useState(!1),[d,l]=f.useState(null);Q.useEffect(()=>{!n.connected&&!n.connecting&&(console.log("Auto-connecting to FICS..."),n.connect())},[n]),Q.useEffect(()=>{n.error&&r.addMessage("console",{channel:"console",sender:"System",content:`Error: ${n.error}`,timestamp:new Date,type:"system"})},[n.error,r]);const h=u=>{if(console.log("handleSendMessage called with:",u,"Length:",u.length),!u.trim())return;const b=u.split(`
